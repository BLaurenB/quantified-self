...
/***/ (function(module, exports, __webpack_require__) {
	...
	var getFoodList = function getFoodList() {
	  ...
	  fetch('https://qs-backend-express.herokuapp.com/api/v1/foods').then(function (response) {
	    return response.json();
	  }).then(parseAndAppendFoods).catch(errorLog);
	};
...
/***/ (function(module, exports, __webpack_require__) {
	...
	$('#food-filter').on('keyup', function (event) {
	  var input = event.currentTarget.value.toUpperCase();
	  var i = input.length;
	  var listItems = Array.from($('#food-list-meals').find('tr'));
	  listItems.forEach(function (item) {
	    if (item.textContent.trim().toUpperCase().substr(0, i) != input) {
	      item.style.display = 'none';
	    } else {
	      item.style.display = '';
	    }
	...
	$('.add-to-meal-btns button').on('click', function (event) {
	  event.preventDefault();
