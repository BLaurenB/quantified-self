...
$('#food-list').on('click', '.delete-food-button', function (event) {
  ...
  findFoodsInMeals(event.currentTarget.parentElement.id, parent)
})

$('.meal-table').on('click', '.delete-meal-food-button', function (event) {
  event.preventDefault()
  let foodId = event.currentTarget.dataset.id
  let mealId = event.currentTarget.parentElement.dataset.id
...
$('#food-filter').on('keyup', function (event) {
  let input = event.currentTarget.value.toUpperCase()
  let i = input.length
  let listItems = Array.from($('#food-list-meals').find('tr'))
  listItems.forEach(function (item) {
    if (
