import {mealTotalCalories,
        colorCals,
        toatsCals,
        orderById} from './shared-functions'
...
const getFoodListMeal = () => {
  ...
  fetch('https://vast-retreat-17218.herokuapp.com/api/v1/foods')
    .then(response => response.json())
    .then(myJson => {
      myJson.sort(orderById).forEach(function (food) {
        $('#food-list-meals').append(foodTableRowHtml(food))
      })
    })
}

// Html for food and meals page
