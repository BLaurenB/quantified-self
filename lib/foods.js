...
$('#food-list').on('click', '.delete-food-button', function (event) {
  ...
  console.log('begin find meals')
  findFoodFromMeals(event.target.id)
  // console.log('end find meals, begin delete food')
  // deleteFoodCall(event.target.id)
  // console.log('end delete food')
})
  ...
  fetch('https://vast-retreat-17218.herokuapp.com/api/v1/meals')
    ...
    .then(parsedResponse => {
      parsedResponse.forEach(function (meal) {
        console.log('parsing meals - line 55')
        meal.foods.forEach(function (food) {
          if (food.id == foodId) {
            deleteFoodFromMeals(meal.id, food.id)
            console.log(`sucessfully removed food from meal`)
          }
        })
      })
    })
    .then(() => {
      console.log('food deleted from all meals - line 65, begin delete food')
      deleteFoodCall(foodId)
    })
}

const deleteFoodCall = (id) => {
  console.log('delete food call - line 71')
  return fetch(`https://vast-retreat-17218.herokuapp.com/api/v1/foods/${id}`, {
    method: 'DELETE'
  }).then(() => console.log('delete food call complete - line 74'))
